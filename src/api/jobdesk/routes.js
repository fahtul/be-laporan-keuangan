...
const JOBDESK_STORAGE_PATH = path.resolve(
  __dirname,
  process.env.JOBDESK_READ_STORAGE_PATH
);
...
const handleFileRequest = async (request, h) => {
  ...
  const filePath = path.join(JOBDESK_STORAGE_PATH, fileName);
  if (!isPathWithinDirectory(JOBDESK_STORAGE_PATH, filePath)) {
...
const routes = (handler) => [
  {
    method: "POST",
    path: "/jobdesk",
    handler: handler.postJobdeskWithFileHandler,
    options: {
      auth: "jims_jwt",
      payload: {
  ...
  },
  {
    method: "GET",
    path: "/jobdesk",
    handler: (request, h) => handler.getJobdeskHandler(request, h),
    options: {
      auth: "jims_jwt",
    },
  },
